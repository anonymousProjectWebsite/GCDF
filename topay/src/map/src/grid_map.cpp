...
namespace nmoma_planner
{
    void GridMap:: init(ros::NodeHandle& nh)
    {
        ...
        bool use_map_node = (mode.compare("planner") == 0 || mode.compare("planner_tro_exp") == 0);
        if (!use_map_node)
        {
    ...
    void GridMap::cloudCallback(const sensor_msgs::PointCloud2 msg)
    {
        ...
        for (size_t i=0; i<pc.points.size(); i++)
        {
            ...
            if (isInMap2d(id))
            {
                ...
                if (pc.points[i].z < moma_param.chassis_height + 0.4)
    ...
    void GridMap::visCallback(const ros::TimerEvent& /*event*/)
    {
        ...
        if (mode.compare("planner") != 0 && mode.compare("planner_tro_exp") != 0){
