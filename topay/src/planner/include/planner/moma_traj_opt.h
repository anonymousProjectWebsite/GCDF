...
struct MomaTraj
{
        ...
        //  PolyTrajectory<9, 5> // theta, arc, mani(7)
        //  6 theta + arc + 6  = 8 
        PolyTrajectory<8, 5> poly_traj;     // theta, arc, mani(6)
        std::vector<Eigen::Vector4d> car_seq;

        MomaTraj() {}
        MomaTraj(PolyTrajectory<8, 5> ploy_traj_, 
                    ...
                    const RowMatrixXd coeffsM)
        {
            ...
            std::vector<CoefficientMat<8, 5>> coeffs;
            for (int i=0; i<durations.size(); i++)
            {
                durs.push_back(durations(i));
                //  block(i*9, 0, 9, 6) 8
                coeffs.push_back(coeffsM.block(i*8, 0, 8, 6));
            }
            poly_traj = PolyTrajectory<8, 5>(durs, coeffs);
            init();
            return;
        }
        ...
        Eigen::VectorXd getState(double t) const
        {
            ...
            //  tail(7) 6  → tail(6)
            state.tail(6) = poly_traj.getPos(t).tail(6);
        ...
        Eigen::VectorXd getDState(double t) const
        {
            ...
            //  tail(7) tail(6)
            state.tail(6) = poly_traj.getVel(t).tail(6);
        ...
        RowMatrixXd sampleTimePoints(int n) const
        {
            ...
            RowMatrixXd points;
            //  12 = 10 + 2 9 + 2 = 11
            points.resize(points_vec.size(), 11);
            for (size_t i = 0; i < points_vec.size(); i++)
            {
                //  head(10) head(9)
                points.row(i).head(9) = points_vec[i];
        ...
        RowMatrixXd sampleArcPoints(int n) const
        {
            ...
            //  10  9 
            points.resize(points_vec.size(), 9);
            for (size_t i = 0; i < points_vec.size(); i++)
                points.row(i) = points_vec[i].head(9);
            return points;
        }
    };
    ...
    inline bool MomaTrajOpt::printConstraintsSituations(MomaTraj traj)
    {
        ...
        if (fabs(max_vel) > 1.1 * moma_param.max_v)
        {
            feasible = false;
            PRINT_RED(max_vel);
        }
        ...
        if (fabs(max_acc) > 1.1 * moma_param.max_a)
        {
            feasible = false;
            PRINT_RED(max_acc);
        }
        ...
        if (fabs(max_domega) > 1.1 * moma_param.max_w)
        {
            feasible = false;
        ...
        if (fabs(max_d2omega) > 1.1 * moma_param.max_dw)
        {
            feasible = false;
