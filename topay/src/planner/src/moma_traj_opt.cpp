...
namespace nmoma_planner 
{
    bool MomaTrajOpt::optimizeEE(Eigen::VectorXd& moma_pos, const Eigen::VectorXd& ee_ref)
    {
        ...
        Eigen::Map<Eigen::VectorXd> Vq(x.data(), moma_param.kDofNum+3);
        Vq.head(3) = moma_pos.head(3);
        for (int i = 0; i < moma_param.kDofNum; i++)
    ...
    double MomaTrajOpt::eeCostCallback(void* ptrObj, const Eigen::VectorXd& x, Eigen::VectorXd& grad)
    {
        ...
        Eigen::Map<const Eigen::VectorXd> Vq(x.data(), obj.moma_param.kDofNum + 3);
        Eigen::Map<Eigen::VectorXd> gradVq(grad.data(), obj.moma_param.kDofNum + 3);

        Eigen::VectorXd now_pos = Vq;
        for (int i = 0; i < obj.moma_param.kDofNum; i++)
        ...
        for (size_t cidx=0; cidx<colli_pts.size(); cidx++)
        {
            ...
            if (cidx > 2)
            {
                double height = obj.moma_param.chassis_height + 
        ...
        for (size_t j = 0; j < obj.moma_param.kDofNum; j++)
                                    ...
                                    const Eigen::MatrixXd& boundary_acc_)
    {
        ...
        sampled_path.clear();
        Eigen::VectorXd state12d = Eigen::VectorXd::Zero(moma_param.kDofNum + 5); // x y theta delta_theta delta_arc, q
        state12d.head(3) = init_path[0].head(3);
        state12d.tail(moma_param.kDofNum) = init_path[0].tail(moma_param.kDofNum);
        ...
        for (size_t i = 1; i<init_path.size(); i++)
        {
            ...
            if (fabs(theta_diff) > 1e-2)
            {
                if (arc_len < 1e-2)
                {
                    ...
                    state12d[4] = 0.0;
                    state12d.tail(moma_param.kDofNum) = init_path[i].tail(moma_param.kDofNum);
                    sampled_path.push_back(state12d);
                }
                else
                {
        ...
        std::vector<Eigen::VectorXd> vector_inner_pts; //  yaw,s,q
        ...
        for(double t = sample_interval; t<total_time-1e-3; t+=sample_interval)
        {
            ...
            for (size_t k = now_idx; k<path_num; k++)
            {
                ...
                if(tmp_arc >= arc)
                {
                    ...
                    Eigen::VectorXd pts = Eigen::VectorXd::Zero(moma_param.kDofNum + 2);
                    pts(0) = pre_path_node[2] + (l-l1)/l*(path_node[3]);
                    pts(1) = path_arcs[k-1] + (l-l1)/l*(path_node[4]);
                    pts.tail(moma_param.kDofNum) = pre_path_node.tail(moma_param.kDofNum) + (l-l1)/l*(path_node.tail(moma_param.kDofNum) - pre_path_node.tail(moma_param.kDofNum));
        ...
        //! NOTE: now tail va = 0
        // start pva
        minco_start_state = Eigen::MatrixXd::Zero(moma_param.kDofNum + 2, 3);
        ...
        minco_start_state.col(0).tail(moma_param.kDofNum) = sampled_path[0].tail(moma_param.kDofNum);
        minco_start_state.col(1).tail(moma_param.kDofNum) = boundary_vel_.col(0).tail(moma_param.kDofNum);
        minco_start_state.col(2).tail(moma_param.kDofNum) = boundary_acc_.col(0).tail(moma_param.kDofNum);

        // end pva
        minco_end_state = Eigen::MatrixXd::Zero(moma_param.kDofNum + 2, 3);
        minco_end_state(0, 0) = sampled_path.back()[2];
        minco_end_state(1, 0) = path_arcs.back();
        minco_end_state.col(0).tail(moma_param.kDofNum) = sampled_path.back().tail(moma_param.kDofNum);
        minco_end_state.col(1).tail(moma_param.kDofNum) = boundary_vel_.col(1).tail(moma_param.kDofNum);
        minco_end_state.col(2).tail(moma_param.kDofNum) = boundary_acc_.col(1).tail(moma_param.kDofNum);

        // GO!
        piece_num = vector_inner_pts.size() + 1;
        times = Eigen::VectorXd::Constant(piece_num, sample_interval);
        inner_pts = Eigen::MatrixXd::Zero(moma_param.kDofNum + 2, piece_num-1);
        ...
        int variable_num = (moma_param.kDofNum + 2) * (piece_num - 1) + piece_num + 1;
        Eigen::VectorXd x;
        x.resize(variable_num);
        int opt_var_idx = 0;
        ...
        opt_var_idx += piece_num;
        Eigen::Map<Eigen::MatrixXd> Vq(x.data()+opt_var_idx, moma_param.kDofNum, piece_num-1);
        for (int i = 0; i < piece_num - 1; i++)
        {
            Tau(i) = logC2(times(i));
            Theta(i) = inner_pts(0, i);
            Arc(i) = inner_pts(1, i);
            for (size_t j = 0; j < moma_param.kDofNum; j++)
        ...
        if (success)
        {
            ...
            opt_var_idx += piece_num;
            Eigen::Map<const Eigen::MatrixXd> Q(x.data()+opt_var_idx, moma_param.kDofNum, piece_num-1);
    ...
    double MomaTrajOpt::firstStageCostCallback(void* ptrObj, const Eigen::VectorXd& x, Eigen::VectorXd& grad)
    {
        ...
        opt_var_idx += obj.piece_num;
        Eigen::Map<const Eigen::MatrixXd> Vq(x.data()+opt_var_idx, obj.moma_param.kDofNum, obj.piece_num-1);
        Eigen::Map<Eigen::MatrixXd> gradVq(grad.data()+opt_var_idx, obj.moma_param.kDofNum, obj.piece_num-1);

        obj.calTfromTau(Tau, obj.times);
        obj.minco_end_state(1, 0) = Arc[obj.piece_num-1];
        obj.inner_pts.resize(obj.moma_param.kDofNum + 2, obj.piece_num-1);
        ...
        Eigen::MatrixXd gradQ = gdP.bottomRows(obj.moma_param.kDofNum);
        for (int i = 0; i < obj.piece_num-1; i++)
            for (size_t j = 0; j < obj.moma_param.kDofNum; j++)
    ...
    {
        cost = 0.0;
        gdC.resize(6*piece_num, moma_param.kDofNum + 2);
        gdC.setZero();
        ...
        {
            const Eigen::Matrix<double, 6, moma_param.kDofNum + 2> &c = minco_opt.getCoeffs().block<6, moma_param.kDofNum + 2>(6*i, 0);
        ...
        for (int i = 0; i < piece_num; i++)
        {
            ...
            for (int j=0; j<=inner_num; j++)
            {
                if (j % 2 == 0)
                {
                    ...
                    // manipulator
                    gradBeta = Eigen::MatrixXd::Zero(3, moma_param.kDofNum);
                    // manipulator environment collision
                    Eigen::VectorXd moma_pos = Eigen::VectorXd::Zero(3+moma_param.kDofNum);
                    moma_pos.head(2) = CurrentXY;
                    moma_pos(2) = state(0);
                    moma_pos.tail(moma_param.kDofNum) = state.tail(moma_param.kDofNum);
                    ...
                    // joint pos limit
                    for (size_t ji = 0; ji < moma_param.kDofNum; ji++)
                    {
                    ...
                    gradBeta.block<1, moma_param.kDofNum>(0, 0) = moma_grad.tail(moma_param.kDofNum);
                    gdT(i) += moma_grad.tail(moma_param.kDofNum).dot(dstate.tail(moma_param.kDofNum)) * real_alpha;
                                        
                    // joint vel and acc
                    Eigen::VectorXd dq = dstate.tail(moma_param.kDofNum);
                    Eigen::VectorXd d2q = d2state.tail(moma_param.kDofNum);
                    Eigen::VectorXd d3q = d3state.tail(moma_param.kDofNum);
                    ...
                    for (size_t jidx = 0; jidx < moma_param.kDofNum; jidx++)
                    {
                        {
                            double violaDqPena;
                            double violaDqPenaD;
                    ...
                    gdC.block<6, moma_param.kDofNum>(i*6, 2) += beta0 * gradBeta.row(0) 
                                               + beta1 * gradBeta.row(1) 
                                               + beta2 * gradBeta.row(2);
                }
                else
                                    ...
                                    const double fx, const double step, int k, int ls)
    {
        ...
        Inner_pts.resize(obj.moma_param.kDofNum + 2, obj.piece_num-1);
