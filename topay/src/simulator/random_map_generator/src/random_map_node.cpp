...
// // folder name (subfolder under simulator/random_map_generator/env/) for file-case PCDs
...
void generateFileCase()
{
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloudFiltered(new pcl::PointCloud<pcl::PointXYZ>);
        std::string pcd_path = ros::package::getPath("random_map_generator")+"/env/" + pcd_folder + "/map.pcd";
        ROS_INFO("Loading PCD file: %s", pcd_path.c_str());
        if (pcl::io::loadPCDFile<pcl::PointXYZ>(pcd_path, *cloudFiltered) == -1)
    {
    ...
    vg.filter(cloud_map);
    // lifting the points a bit on z-axis
    for (size_t i=0; i<cloud_map.points.size(); i++){
        cloud_map.points[i].z += 0.4;
        if (cloud_map.points[i].x < 1.0 && cloud_map.points[i].x > -1.0 &&
           cloud_map.points[i].y < 1.0 && cloud_map.points[i].y > -1.0)
        {
            cloud_map.points[i].x += 200.0;
        }
    }

    // add boundary walls (same logic as other cases so file maps have consistent walls)
    // pcl::PointCloud<pcl::PointXYZ> cloud_box = generateBox(Eigen::Vector3d(size_x, resolution*2.0, 1.0));
    // pcl::PointXYZ pt_random;
    // for (size_t i=0; i<cloud_box.points.size(); i++)
    // {
...
int main (int argc, char** argv)
{
    ...
    // optional: folder under env/ where map.pcd resides for case_id == 2
    nh.getParam("/map/pcd_folder", pcd_folder);
