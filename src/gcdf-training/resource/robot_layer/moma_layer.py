...
class MoMaLayer(torch.nn.Module):
    def __init__(self, device='cpu',mesh_path = os.path.dirname(os.path.realpath(__file__)) + "/meshes/moma/*.STL"):
        ...
        self.theta_min = torch.tensor([-2.8973, -2.1750, -2.1750, -2.1750, -2.1750, 2.6100,  2.6100]).to(self.device)
        self.theta_max = torch.tensor([ 2.8973,	2.1750,	 2.1750,  2.1750,  2.1750,	2.6100,	 2.6100]).to(self.device)
...
if __name__ == "__main__":
    ...
    # show robot
    # theta = robot.theta_min + (robot.theta_max-robot.theta_min)*0.5
    ...
    robot_mesh = moma.get_forward_robot_mesh(pose, theta)
    robot_mesh = np.sum(robot_mesh)
