...
int main(int argc, char **argv){
  ...
  n.param("map/pcd_folder", map_name, std::string(""));
  ...
  if(_map_type == 0){
    ...
    GenerateBridge();
      ...
      // GenerateFromFiles();
            ...
            std::string pcd_path = ros::package::getPath("plan_env")+"/env/" + map_name + "/map.pcd";
        ...
        for (size_t i=0; i<cloudMap.points.size(); i++){
            cloudMap.points[i].z += 0.4;
            if (cloudMap.points[i].x < 1.0 && cloudMap.points[i].x > -1.0 &&
               cloudMap.points[i].y < 1.0 && cloudMap.points[i].y > -1.0)
            {
                cloudMap.points[i].x += 20.0;
            }
        }
        cloudMap.width = cloudMap.points.size();
        cloudMap.height = 1;
        ...
        kdtreeLocalMap.setInputCloud(cloudMap.makeShared());
        _map_ok = true;
        ROS_WARN("Finished generate Map from pcd files");
