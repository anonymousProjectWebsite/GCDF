...
namespace remani_planner
{
                                       ...
                                       Eigen::VectorXd &g)
  {
    ...
    if(!std::isfinite(costall) || !g.allFinite()){
      double min_t = std::numeric_limits<double>::max();
      double max_t = -std::numeric_limits<double>::max();
      for(const auto &T : T_container){
        min_t = std::min(min_t, T.minCoeff());
        max_t = std::max(max_t, T.maxCoeff());
      }
      ROS_ERROR_STREAM("[PolyTrajOptimizer] Non-finite cost/grad detected. "
                       << "cost=" << costall
                       << " minT=" << min_t
                       << " maxT=" << max_t
                       << " total_smooth=" << total_smoo_cost
                       << " total_obs_feas=" << total_obs_feas_costs.sum()
  ...
  bool PolyTrajOptimizer::feasibilityGradCostCar(const Eigen::Vector2d &vel, const Eigen::Vector2d &acc, const Eigen::Vector2d &jer, const int trajid,
    ...
    const double vTv_raw = vel.squaredNorm();
    const double vTv = std::max(vTv_raw, 1e-6); // guard zero speed
    const double aTv = acc.transpose() * vel;
    const double aTBv = acc.transpose() * B_h_ * vel;
    const double jTBv = jer.transpose() * B_h_ * vel;
    const double v_norm = std::max(std::sqrt(vTv), non_singul_v_);
