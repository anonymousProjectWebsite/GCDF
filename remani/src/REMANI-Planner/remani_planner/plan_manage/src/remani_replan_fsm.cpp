...
namespace remani_planner
{
  ...
  void REMANIReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    /*  fsm param  */
    nh.param("/map_generator/map/pcd_folder", map_name, std::string(""));
  ...
  void REMANIReplanFSM::waypointCallback(const geometry_msgs::PoseStamped::ConstPtr &msg){
    std::string goal_path = ros::package::getPath("plan_env")+"/env/" + map_name + "/goal_list.txt";
    ...
    int num_goal = 10;
    {
      ...
      if (!goal_file.is_open()) {
        ROS_ERROR("[BatchPlan] Cannot open goal file: %s", goal_path.c_str());
        return;
      }

      std::string line;
      while (std::getline(goal_file, line) && (int)goal_list.size() < num_goal)
      {
        if (line.empty()) continue;
        std::istringstream iss(line);

        // 3 + manipulator_dim_ : x y yaw + joints
        Eigen::VectorXd goal_state(3 + manipulator_dim_);
        goal_state.setZero();

        for (int i = 0; i < 3 + manipulator_dim_; i++) {
            iss >> goal_state(i);
          }
    ...
    // use same all zero initial configuration
    static Eigen::VectorXd fixed_start_pt;
    static Eigen::VectorXd fixed_start_vel;
    static Eigen::VectorXd fixed_start_acc;
    static Eigen::VectorXd fixed_start_jer;
    static double fixed_start_yaw = 0.0;
    static int fixed_start_singul = 1;
    static bool fixed_start_gripper = false;

    fixed_start_pt.resize(traj_dim_);
    fixed_start_vel.resize(traj_dim_);
    fixed_start_acc.resize(traj_dim_);
    fixed_start_jer.resize(traj_dim_);
    fixed_start_pt.setZero();
    fixed_start_vel.setZero();
    fixed_start_acc.setZero();
    fixed_start_jer.setZero();
    //  NaN
    fixed_start_vel(0) = mobile_base_non_singul_vel_ * cos(fixed_start_yaw);
    fixed_start_vel(1) = mobile_base_non_singul_vel_ * sin(fixed_start_yaw);
    // taget waypoints
    Eigen::VectorXd local_target_vel = Eigen::VectorXd::Zero(traj_dim_);
    Eigen::VectorXd local_target_acc = Eigen::VectorXd::Zero(traj_dim_);
    bool local_target_gripper = false;

    std::vector<bool> succ_list(goal_list.size(), false);
    std::vector<double> time_list(goal_list.size(), 0.0);
    std::vector<double> dist_translate_list(goal_list.size(), 0.0);
    std::vector<double> dist_config_list(goal_list.size(), 0.0);

    auto wrapToPi = [](double ang){
      return atan2(sin(ang), cos(ang));
    };

    if (goal_list.empty()){
      ROS_WARN("[BatchPlan] goal list is empty, skip planning.");
      return;
    }

    for (size_t i = 0; i < goal_list.size(); ++i){
      Eigen::VectorXd goal_pt = Eigen::VectorXd::Zero(traj_dim_);
      goal_pt.head(2) = goal_list[i].head(2);
      for(int j = 0; j < manipulator_dim_; ++j){
        goal_pt(mobile_base_dim_ + j) = goal_list[i](3 + j);
      }
      double goal_yaw = goal_list[i](2);
      // 
      Eigen::Vector2d dir = goal_pt.head(2) - fixed_start_pt.head(2);
      Eigen::VectorXd local_target_vel = Eigen::VectorXd::Zero(traj_dim_);
      if(dir.norm() > 1e-3){
        local_target_vel.head(2) = mobile_base_non_singul_vel_ * dir.normalized();
      }else{
        local_target_vel(0) = mobile_base_non_singul_vel_;
      }
      Eigen::VectorXd local_target_acc = Eigen::VectorXd::Zero(traj_dim_);
      double init_time_ms = 0.0, opt_time_ms = 0.0;
      std::cout << "Batch planning to goal " << i << ": " << goal_pt.transpose() << ", yaw: " << goal_yaw << std::endl;

      ros::Time start_time = ros::Time::now();
      bool success = planner_manager_->reboundReplan(
        fixed_start_pt, fixed_start_vel,
        fixed_start_acc, fixed_start_jer,
        fixed_start_yaw, fixed_start_singul, fixed_start_gripper,
        ros::Time::now().toSec(),               // trajectory_start_time
        goal_pt, local_target_vel, local_target_acc,
        goal_yaw, local_target_gripper,
        /*flag_polyInit=*/true,                 //  A* init
        /*flag_randomPolyTraj=*/false,
        /*have_local_traj=*/false,              //  goal  fresh solve
        init_time_ms, opt_time_ms);
      time_list[i] = (ros::Time::now() - start_time).toSec();
      succ_list[i] = success;
      if(!success){
        ROS_ERROR("Batch planning failed at goal %d!", i);
        continue;
      }else{
        //  batch 
        sendPolyTrajROSMsg();
        ...
        local_path_list.reserve(i_end);
        for(int k = 0; k < i_end; ++k){
          Eigen::Vector2d local_traj_pt = planner_manager_->traj_container_.singul_traj_data.getPos(k * 0.02).head(2);
        ...
        // 
        const auto &traj = planner_manager_->traj_container_.singul_traj_data;
        double duration = traj.duration;
        if(duration > 1e-3){
          int num_samples = 40;
          double dt = duration / (num_samples - 1);
          Eigen::Vector2d prev_xy;
          Eigen::VectorXd prev_cfg(1 + manipulator_dim_);
          double dist_trans = 0.0, dist_cfg = 0.0;
          for(int s = 0; s < num_samples; ++s){
            double t = std::min(duration, s * dt);
            Eigen::VectorXd pos = traj.getPos(t);
            double yaw = traj.getCarAngle(t);
            Eigen::Vector2d xy = pos.head(2);
            Eigen::VectorXd cfg(1 + manipulator_dim_);
            cfg(0) = yaw;
            cfg.tail(manipulator_dim_) = pos.tail(manipulator_dim_);
            if(s > 0){
              dist_trans += (xy - prev_xy).norm();
              Eigen::VectorXd dcfg = cfg - prev_cfg;
              dcfg(0) = wrapToPi(cfg(0) - prev_cfg(0));
              dist_cfg += dcfg.norm();
            }
            prev_xy = xy;
            prev_cfg = cfg;
          }
          dist_translate_list[i] = dist_trans;
          dist_config_list[i] = dist_cfg;
        }
      }
    }

    std::string result_path = ros::package::getPath("plan_env")+"/env/" + map_name + "/remani_results.txt";
    std::ofstream result_file(result_path);
    if(!result_file.is_open()){
      ROS_ERROR("Cannot open result file: %s", result_path.c_str());
    }else{
      for (size_t i = 0; i < goal_list.size(); i++)
      {
        result_file << succ_list[i] << " " << time_list[i] << " "
                    << dist_translate_list[i] << " "
                    << dist_config_list[i] << std::endl;
      }
      result_file.close();
      ROS_INFO("Saved batch planning results to %s", result_path.c_str());
    }
    }
